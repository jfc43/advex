...
parser.add_argument('--hparams', default='spambase-1layer-adv.yaml',
                    type=str,
...
def run_exp(params):
  ...
  feature_columns = []
  # for spambase we know the cols are all numerical, so we're not being too
  # careful here
  for col in specs:
    feature_columns.append(
      feature_column.numeric_column(col['name']))
...
# In[26]:
  ...
  def train_step_perturb_input(examples, labels, eps, eps_step, num_pgd_steps):
    ...
    features_prob_functional(features) # dummy call to force weights to be created
...
# In[29]:
  ...
  model_file = 'spambase_models/' + f'eps={eps}'
...
ig_init_cols = ['correct', 'ent', 'a1p', 'gini']
