...
class Model(object):
  ...
  def _build_model(self):
    ...
    self.correct_prediction = tf.equal(self.predictions, self.label)
    ...
    self.loss_input_gradient = tf.gradients(self.xent, self.input)[0]
    self.output_input_gradient = tf.gradients(self.output[:,self.label_ph], self.input)[0]

  def forward(self, input):
    with tf.variable_scope('input', reuse=tf.AUTO_REUSE):
      x = self._conv('init_conv', input, 3, 3, 16, self._stride_arr(1))
    ...
    with tf.variable_scope('logit', reuse=tf.AUTO_REUSE):
      output = self._fully_connected(x, 10)
    return output
