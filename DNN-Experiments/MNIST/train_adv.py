...
config_filename = 'adv_config.json'
...
train_step = tf.train.AdamOptimizer(1e-4).minimize(model.sum_loss,
...
tf.summary.scalar('xent adv train', model.sum_loss / batch_size)
tf.summary.scalar('xent adv', model.sum_loss / batch_size)
...
with tf.Session(config = tf_config) as sess:
  ...
  for ii in range(curr_step, max_num_training_steps):
    ...
    if ii % num_output_steps == 0:
      ...
      adv_loss = sess.run(model.sum_loss, feed_dict=adv_dict)
